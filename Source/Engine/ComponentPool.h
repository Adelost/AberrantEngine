...
template<class T>
class ComponentPool : public PoolArray<T>, public AbstractComponentPool
{
public:
	void clear()
	{
...
// 	{
// 		int index = PoolArray::aComponentPooldd(value);
// 		if (index == m_enabled.count())
// 		{
// 			m_enabled.add();
// 			m_ids.add();
// 		}
...
// 	}
	...
	{
		xassert_msg(indexFromId(id) == -1,
		            "entity already has specified component");

		int index = m_firstGap;
		...
		// Save id to be able to retrieve
		// values later
	...
	/// Returns index belonging to an id, returns "-1" if no index exists
